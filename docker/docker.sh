...
kubeadm init --token=acbec6.2852dff7cb569aa0 --apiserver-advertise-address=`0.0.0.0` --apiserver-cert-extra-sans=`instakube,instakube.127.0.0.1.xip.io` --skip-preflight-checks

#kubeadm init --token=acbec6.2852dff7cb569aa0 --skip-preflight-checks
...
# kubectl config set-cluster instakube --server=http://127.0.0.1:8080
# kubectl config set-context instakube --cluster=instakube
# kubectl config use-context instakube

# /usr/bin/kubectl --kubeconfig=/etc/kubernetes/admin.conf proxy --disable-filter=true --api-prefix=/ --address=0.0.0.0 --port 8080